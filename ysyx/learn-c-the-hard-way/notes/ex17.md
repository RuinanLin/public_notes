...
因为改过的`ex17`已经不是严格存储结构体了，而是存储了结构体中的指针指向的内容，所以此处分析的是原版的`ex17.c`。
...
- `int set`：4字节；
- `char name[512]`：512字节；
- `char email[512]`：512字节。

而这些已经对齐了，直接累加即可，总共1032字节，于是`struct Database`总共103200字节。在操作系统中查看文件大小，正好是103200字节。

下面试试在结构体的一开始增加一个字节，形成`ex17-4.c`：
...
一个`Address`总共1036个字节，正好对应文件大小103600字节。

### 向`Address`添加一些字段，使它们可被搜索。

这个部分和第三题重了，一个道理。

### 编写一个shell脚本来通过以正确顺序运行命令执行自动化测试。提示：在`bash`顶端使用`set -e`，使之在任何命令发生错误时退出。

已经写好了脚本`ex17-6.sh`，实现了随机的命令生成和自动检查。里面的注释、代码结构很清晰，这里不再细致说明。

### 尝试重构程序，使用单一的全局变量来储存数据库连接。这个新版本和旧版本比起来如何？

没看懂这个问题是什么意思，什么叫“比起来如何”？
